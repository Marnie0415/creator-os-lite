      ...
      MyApplicationTheme(isDarkTheme = isDarkMode) {
        ...
        val ghostedAndOverdue = remember(dashboardState.riskFeed) {
          ...
          for (risk in dashboardState.riskFeed) {
            when (risk) {
              is RiskItem.CriticalClient -> map.add(risk.clientName to (risk.pendingAmount to stringResource(R.string.dash_badge_critical)))
              is RiskItem.OverdueInvoice -> map.add(risk.projectTitle to (risk.amount to stringResource(R.string.dash_badge_high)))
              is RiskItem.GhostedClient -> map.add(risk.clientName to (0.0 to "${risk.hoursSinceContact} hr ghosted"))
              else -> {}
            }
          }
          map
